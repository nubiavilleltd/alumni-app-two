...
| Goal 4 | In progress | Nine SQL-backed auth/recovery/verification routes; self-only access-code verification; canonical database-fact permission policy in ADR 0004; lower-role account-management hierarchy applied to approval, account state, and bounded role changes; transactional refresh-token revocation; sixteen retired tombstones; bcrypt-to-Argon2id migration; asymmetric JWTs; rotating hashed refresh tokens; finite reset/email codes; shared Redis throttling. Social login is now implemented (`/socials/social_login|social_signup|link|unlink`) with server-side Google/Facebook token verification and auto-link/signup; `tests/test_social.py` (6 tests). | Negative protected-route tests across every route family, live Redis/SMTP acceptance, trusted-proxy validation, and full frontend compatibility remain. | 2026-09-18 |
...
| Goal 9 | In progress | Unsafe legacy `user_tokens` blocked by tested 410 responses. `GET|POST /api/get_notifications` and `POST /api/mark_notification_read` are implemented (self-only, bounded, current-schema). `POST /api/contact_us` implemented (public, rate-limited, persisted, manager-notified). `POST /api/create_notification` implemented (content-administrator only, one reviewed per-user row). VAPID Web Push implemented (`GET /chat_api/get_vapid_key`, `POST /chat_api/register_push_subscription`, `app/integrations/push.py` + `PushService.notify`); the PHP Expo SDK is not retained (no Expo-token table in the reviewed schema). `tests/test_notifications.py` (3) + `tests/test_push.py` (3). | Live VAPID keys and SMTP credentials, and any mobile-Expo provider decision, remain. | 2026-09-18 |
| Goal 10 | In progress | Typed trusted-host/public-base-url config with production wildcard guards; CORS allowlist middleware (`allow_credentials=False`); `SecurityHeadersMiddleware`; per-route `enforce_rate_limit`; cookie/CSRF documented N/A; recursive nested/PII/payment-payload log redaction; gated `/metrics` endpoint + `MetricsMiddleware`; threat model in `docs/threat-model.md`; additive migration `e7f8a9b0c1d2` adds a secret-free `audit_log` table wired into account management; retention policy in `docs/retention-policy.md`. `tests/test_security.py` (6) + `tests/test_metrics.py` (5) + `tests/test_audit.py` (1). | CI dependency/container scanning (Goal 12), a reviewed self-service data-deletion flow, and per-domain metrics counters remain. | 2026-09-18 |
...
#### 2026-09-18 — Goal 4 social login, Goal 9 notifications/push, Goal 10 audit + retention, retirements

- **Retirements (Goal 7).** Added HTTP 410 tombstones for the six unused `privacy_policy` and legacy `market` routes (static page / no table / no caller).
- **Social login (Goal 4).** `app/integrations/social.py` verifies Google (tokeninfo + audience + email-verified) and Facebook (debug_token + app_id + `/me`) tokens server-side; `AuthService.social_login`/`social_signup`/`social_link`/`social_unlink` map verified identities to accounts (auto-link existing email, create a minimal `email_verified=1`/`has_password=0` account on signup), and `app/api/social.py` exposes `/socials/social_login|social_signup|link|unlink` (login/signup public + rate-limited; link/unlink authenticated). Credentials load from `ALUMNI_GOOGLE_CLIENT_ID`, `ALUMNI_FACEBOOK_APP_ID`, `ALUMNI_FACEBOOK_APP_SECRET`. `tests/test_social.py` (6 tests).
- **Notifications (Goal 9).** `POST /api/create_notification` (content-administrator only, current-database `MANAGE_CONTENT`) inserts one reviewed per-user `notifications` row; caller-selected recipients and the reusable key are not copied. `tests/test_notifications.py` (3 tests).
- **Push (Goal 9).** VAPID Web Push over the reviewed `user_push_subscriptions` schema: `GET /chat_api/get_vapid_key` and `POST /chat_api/register_push_subscription` plus `app/integrations/push.py` (pywebpush) and `PushService.notify`. The PHP Expo SDK is not retained (no Expo-token table in the reviewed schema). `tests/test_push.py` (3 tests).
- **Audit (Goal 10).** Additive reversible migration `e7f8a9b0c1d2` adds a secret-free `audit_log` table; `AuditRepository.record` is wired into `manage_member_account` (role/state changes). `tests/test_audit.py` (1 test). Retention policy documented in `docs/retention-policy.md`.
- **Hidden callers.** Static sweep of the frontend found no caller for the retired routes or the notification/push endpoints (external systems require production access, deferred).
- **Result.** Full suite **552 passed, 0 failed, 90.27% coverage**; `pip check`, Ruff format/check (155 files), strict `mypy` (155 files), Bandit, and `alembic check` clean at head `e7f8a9b0c1d2`.
